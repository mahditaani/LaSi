...
bool wlsQE(double l){//Simulates the QE of the WLS rejecting hits accordingly.
//    std::cout << "Lambda: "<< l<<std::endl;
    std::default_random_engine generator(rand());
    std::uniform_real_distribution<double> distribution(0.0, 100);
    double rand = distribution(generator);
...
//        std::cout <<"Lambda: " << _muon.GetPhoton(i).lambda()<<std::endl;
        double xVal = hyp*cos(i*angleDiff)+ _muon.GetVtx(0);
        //std::cout <<"xvalPhot: " << xVal <<std::endl;
        double yVal = hyp*sin(i*angleDiff)+_muon.GetVtx(1);
        //std::cout <<"yvalPhot: " << yVal <<std::endl;
...
//        if (withWLS){
        if ( InCircle(point, pmtRadius, pmtPos[0], pmtPos[1]) && pmtQE(_muon.GetPhoton(i).lambda()))
            // Add detected virtual hit if WLS and in the Square (WLS plate).
        {detected d; d.wavelength = _muon.GetPhoton(i).lambda(); d.material = "PMT"; detectPhots.push_back(d);}
        else if ( !InCircle(point, pmtRadius, pmtPos[0], pmtPos[1]) && InSquare(point, _wls.GetLength(0), pmtPos[0], pmtPos[1]) && _wls.Absorb(_muon.GetPhoton(i).lambda()) && _wls.QE())
            // Add detected virtual hit if WLS and in the Square (WLS plate).
        {detected d; d.wavelength = _muon.GetPhoton(i).lambda(); d.material = "WLS"; detectPhots.push_back(d);}
